...
class TestTournamentUrlSlug:
    ...
    def test_multi_word_city(self):
        t = Tournament(
            tournament_id=100, year=2026, city="Buenos Aires", circuit=Circuit.TOUR
        )
        assert t.url_slug == "buenos-aires"

    def test_named_tournament(self):
        t = Tournament(
            tournament_id=580, year=2026, city="Melbourne", circuit=Circuit.TOUR
        )
        assert t.url_slug == "australian-open"

    def test_apostrophe(self):
        t = Tournament(
            tournament_id=999, year=2026, city="Queen's Club", circuit=Circuit.TOUR
        )
        assert t.url_slug == "queens-club"

    def test_hyphen_preserved(self):
        t = Tournament(
            tournament_id=999, year=2026, city="Saint-Tropez", circuit=Circuit.TOUR
        )
...
class TestTournamentPath:
    ...
    def test_name_with_spaces(self):
        ...
        t = Tournament(
            tournament_id=9210, year=2026, city="Geneva", circuit=Circuit.TOUR
        )
